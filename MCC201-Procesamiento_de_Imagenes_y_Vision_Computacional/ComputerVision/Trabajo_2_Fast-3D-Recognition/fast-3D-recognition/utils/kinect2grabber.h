...
string pathPCL = "../pointclouds/";
string pathParams = "../params/";

int WIDTH = 650;
int HEIGHT = 550;

cv::Size newsz = cv::Size(WIDTH, HEIGHT);
...
public:
  ...
  K2G(Processor p = CPU, bool mirror = false, string serial = string()): mirror_(mirror), listener_(libfreenect2::Frame::Color | libfreenect2::Frame::Ir | libfreenect2::Frame::Depth), 
                                         undistorted_(512, 424, 4), registered_(512, 424, 4), big_mat_(1920, 1082, 4), qnan_(numeric_limits<float>::quiet_NaN()){

    signal(SIGINT,sigint_handler);

    if(freenect2_.enumerateDevices() == 0){
      cout << "no kinect2 connected!" << endl;
      exit(-1);
    }

    switch (p){
      case CPU:
        cout << "creating Cpu processor" << endl;
        if (serial.empty())
...
#ifdef WITH_OPENCL
      case OPENCL:
        cout << "creating OpenCL processor" << endl;
        if(serial.empty())
...
#endif
      case OPENGL:
        cout << "creating OpenGL processor" << endl;
        if (serial.empty())
...
#ifdef WITH_CUDA
      case CUDA:
        cout << "creating Cuda processor" << endl;
        if(serial.empty())
  ...
  void printParameters(){
    ...
    cout << "rgb fx=" << cp.fx << ",fy=" << cp.fy <<
      ",cx=" << cp.cx << ",cy=" << cp.cy << endl;
    libfreenect2::Freenect2Device::IrCameraParams ip = getIrParameters();
    cout << "ir fx=" << ip.fx << ",fy=" << ip.fy <<
      ",cx=" << ip.cx << ",cy=" << ip.cy <<
      ",k1=" << ip.k1 << ",k2=" << ip.k2 << ",k3=" << ip.k3 <<
      ",p1=" << ip.p1 << ",p2=" << ip.p2 << endl;
  }
  ...
  void storeParameters(){
    ...
    cout << "storing " << serial_ << endl;
    cv::FileStorage fs(pathParams+"calib_" + serial_ + ".yml", cv::FileStorage::WRITE);
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr updateCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud){
    ...
    const size_t w = undistorted_.width;
    const size_t h = undistorted_.height;
  ...
  void get(cv::Mat & color_mat, cv::Mat & depth_mat, cv::Mat & ir_mat, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, 
...
private:
  ...
  {  
    ...
    unsigned int now = (unsigned int)chrono::duration_cast<chrono::milliseconds>(tnow.time_since_epoch()).count();
    if(!file_streamer_){
      file_streamer_ = new ofstream();
      file_streamer_->open ("stream" + to_string(now), ios::binary);
