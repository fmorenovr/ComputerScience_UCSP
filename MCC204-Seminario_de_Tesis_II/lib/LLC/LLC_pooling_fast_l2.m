...
function [beta] = LLC_pooling_fast_l2(feaSet, B, pyramid, knn)
...
for i = 1:num_block
    ...
    llc_codes(:,block_start:block_end) = sparse(LLC_coding_appr_fast2(B', feaSet.feaArr(:,block_start:block_end)', knn)');
end
    %llc_codes = llc_codes';
...
for iter1 = 1:pLevels,
    ...
    for iter2 = 1:nBins,     
        ...
        beta(:, bId) = sqrt(sum(llc_codes(:, sidxBin) .* llc_codes(:, sidxBin), 2));
    end
end
