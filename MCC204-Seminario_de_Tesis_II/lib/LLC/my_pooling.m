...
function [beta] = my_pooling(feaSet, B, pyramid, knn, options)
...
if options.coding_type==1
    llc_codes = temp_coding(B', feaSet.feaArr', knn, options.binarize);
elseif type==2
    llc_codes = temp_coding2(B', feaSet.feaArr', knn, options.binarize);
end
llc_codes = llc_codes';
