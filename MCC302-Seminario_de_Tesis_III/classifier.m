...
for year_ind = 1 : length(years)
  ...
  config.splits_path = [config.homedir '/outputs/' num2str(config.data_year) '/classification/'  ];
  ...
  config.output_path = [config.homedir '/outputs/' num2str(config.data_year) '/classification/'];
  ensuredir(config.output_path);
  
  for feature_ind = 1 : length(features)
    config.feature_type = features{feature_ind};
    
    config.results_path = [config.output_path config.feature_type];
    ...
    %        end
            ...
            % Now run testing and present _aucs', 'delta_auresults in a webpage.
            ...
            % Plot output results.
            figure_path = sprintf('%s/%s/%s/delta_%d/%d_pr.jpg', config.results_path, city_identifier, metric_str, round(100 * delta), split_id);
            ...
            f = fopen(sprintf('%s/%s/%s/delta_%d/%d.html', config.results_path, city_identifier, metric_str, round(100 * delta), split_id), 'w');
        ...
        namesave = [config.results_path, "/", city_identifier, "/", metric_str, '/results.mat'];
        ...
        print('-dpng', '-r60', sprintf('%s/%s/%s/results.png', config.results_path, city_identifier, metric_str));
        close;
        fprintf(ff, '<h3>_%s_%s</h3>', city_identifier, metric_str);
