...
batch_s, name_loss, opt_name, opt_lr = 128, "epsilon_sensitive", "adam", 0.00099
...
dir_to_save = "outputs/dcnn/regressor/"
...
for data_year in years:
  ...
  for city in cities:
    ...
    for metric in metrics:
      ...
      name_dir = name_dir_ + city+"/"+metric+"/dcnn/"
      verifyDir(name_dir)
      ...
      X_ = genfromtxt(features_dir+"vgg16"+"_X_"+data_year+"_"+city+"_"+metric+".csv", delimiter=',')
      Y_ = genfromtxt(features_dir+"vgg16"+"_y_"+data_year+"_"+city+"_"+metric+".csv", delimiter=',')
        ...
        for split in range(num_splits):
          ...
          for c in C:
            ...
            for alpha_val in alpha:
              model = Input(shape=(4096,))
              cnn = Dense(1, activation="linear", use_bias=False)(model)
              ...
              dcnn_model.compile(optimizer = opt, loss=loss, metrics=['mean_squared_error', 'mean_absolute_error', tf_pearson])
        
              checkpoint_file = delta_dir+"/models/best_alpha_c.h5"
              checkpoints = ModelCheckpoint(checkpoint_file, monitor='tf_pearson', verbose=1, save_best_only=True, mode='max')
              
              lrate = LearningRateScheduler(lr_scheduler, verbose=1)
              
              earlyS = EarlyStopping(monitor='tf_pearson', patience=25, verbose=1, mode='max')
    ...
    for i in range(len(metrics)):
      ...
      plt.grid(True)
      plt.savefig(name_dir_ + city+"/"+metrics[i]+"/dcnn/loss.png")
      plt.clf()
      plt.cla()
      plt.close()
      ...
      #plt.legend([cities[0], cities[1]])
      plt.grid(True)
      plt.savefig(name_dir_ + city+"/"+metrics[i]+"/dcnn/mae.png")
      plt.clf()
      plt.cla()
      plt.close()
      ...
      #plt.legend([cities[0], cities[1]])
      plt.grid(True)
      plt.savefig(name_dir_ + city+"/"+metrics[i]+"/dcnn/pearson.png")
      plt.clf()
      plt.cla()
      plt.close()
